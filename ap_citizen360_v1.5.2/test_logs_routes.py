...
def run_tests():
    ...
    with TestClient(app) as client:
        # 1. Unauthenticated /logs UI
        r = client.get("/logs")
        assert r.status_code == 200
        assert "Runtime Logs Access" in r.text
        assert "6-DIGIT VERIFICATION CODE" in r.text
        print("[PASS] 1. GET /logs renders unauthenticated TOTP login form")

        # 2. Invalid TOTP submission
        r = client.get("/logs?code=000000")
        assert r.status_code == 200
        assert "Invalid or expired 6-digit Authenticator code" in r.text
        print("[PASS] 2. GET /logs?code=000000 rejects invalid code with error")

        # 3. Valid TOTP submission -> sets cookie & returns dashboard
        valid_totp = generate_current_totp(secret)
        r = client.get(f"/logs?code={valid_totp}")
        assert r.status_code == 200
